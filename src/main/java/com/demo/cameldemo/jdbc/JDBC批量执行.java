...
//        非批量执行效率();
    ...
    public void 非批量执行效率() {
        from("timer://非批量执行效率?repeatCount=1")
            ...
            System.out.println(String.format("同步结束,数据量:%s,耗时:%d", exchange.getIn().getHeader("CamelJdbcRowCount"), System.currentTimeMillis() - (long) exchange.getProperty("CostTime")));
        })
                .end()
                .process(ex -> {
    ...
    public void 批量执行效率() {
        ...
        from("timer://批量执行效率?repeatCount=1")
            ...
            System.out.println(String.format("同步结束,数据量:%s,耗时:%d", exchange.getIn().getHeader("CamelJdbcRowCount"), System.currentTimeMillis() - (long) exchange.getProperty("CostTime")));
                ...
                .to("jdbc:49")
                .process(exchange->{
                    System.out.println(String.format("查询结束,数据量:%s,耗时:%d", exchange.getIn().getHeader("CamelJdbcRowCount"), System.currentTimeMillis() - (long) exchange.getProperty("CostTime")));
                })
                .split(body())
//                .setBody(simple("INSERT INTO SYS_USER_COPY(ID, USERNAME, PASSWORD, CONTENT, INFO, DD, TT) VALUES ('${body[ID]}','${body[USERNAME]}','${body[PASSWORD]}','${body[CONTENT]}','${body[INFO]}','${body[DD]}','${body[TT]}')"))
//                .to("sql:INSERT INTO DATAPLATFORM.SYS_USER_COPY(ID, USERNAME, PASSWORD) VALUES ('72521', '张2三', 'admin')?dataSource=db")
                .to("sql:INSERT INTO SYS_USER_COPY(ID, USERNAME, PASSWORD, CONTENT, INFO, DD, TT) VALUES (:#ID,:#USERNAME,:#PASSWORD,:#CONTENT,:#INFO,:#DD,:#TT)?dataSource=183")
//                .to("jdbc:db")
        ;
    }
